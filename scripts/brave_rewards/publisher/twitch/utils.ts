...
const getCurrentMediaId = (): string => {
  if (isVideoPath()) {
    const elem = getChannelTitleElement()
    ...
    return href ? firstPathComponent(href) : ''
  }

  const pathComponent = firstPathComponent(location.pathname).toLowerCase()
  return nonProfilePaths.has(pathComponent) ? '' : pathComponent
}

const getCurrentMediaKey = (mediaId: string): string => {
  if (!mediaId) {
    return ''
  }
...
const getPublisherKey = (mediaId: string): string => {
...
const getCurrentPublisherInfo = (): TwitchPublisherInfo => {
